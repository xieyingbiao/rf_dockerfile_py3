...
class RequestsKeywords(object):
    ...
    def get_request(
        ...
        logger.info('Get Response  : %s' %(response.text.decode('unicode-escape')))
        return response
    ...
    def post_request(
        ...
        logger.info('Post Request using : alias=%s, uri=%s, data=%s, headers=%s, files=%s, allow_redirects=%s '
                    % (alias, uri, data.decode('unicode-escape'), headers, files, redir))
        logger.info('Post Response is %s'%response.text.decode('unicode-escape'))
        return response
